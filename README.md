...
Settings cover accounts (add, remove, show on rail), the rail's display and position — pick a display, then Left, Right, or (for the MacBook's built-in display) Notch; Automatic picks the outer edge of your whole desktop so the pointer rests on the hairline instead of sliding onto the next monitor, and the pane warns when a chosen edge is a seam — plus a Dock-style *Automatically hide the rail* switch (off, the rail stays open on the edge all day, floating over your windows; Left and Right only), auto-hide delay, refresh interval, and launch at login.
